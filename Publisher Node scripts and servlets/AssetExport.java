...
/**
 * Servlet implementation class AssetExport
 */
@WebServlet("/AssetExport")
public class AssetExport extends HttpServlet {
		...
		// TODO Auto-generated method stub
		 JSONObject myData;
			try {
				...
				ProcessBuilder pb = new ProcessBuilder("node", "AssetExport.js", myData.optString("ver"),
						myData.optString("userEmail"));
				java.io.File file = new java.io.File ("C://peekaplatform/assets/temp");
				pb.directory(file);
			...
			try {
				 ...
				 HttpURLConnection peekConn = (HttpURLConnection) url.openConnection();
			        ...
			        if ( peekConn.getResponseCode() == 200 ) {
			        	response.getWriter().write("Asset files have peen pushed to the staging server.");
			        }
			        else {
			        	response.getWriter().write("Asset files have been pushed to s3 and the database has been updated"
